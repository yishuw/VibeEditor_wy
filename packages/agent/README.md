...
所有 Agent 相关类型均在此定义，与外界无依赖：

| 类型 | 说明 |
|------|------|
| `AgentMode` | 工作模式：`'build'`（编码） / `'plan'`（规划） |
| `AgentMessage` | 对话消息（id、role、content、timestamp、editOperations） |
| `AgentEditResult` | 编辑结果（filePath + operations + description） |
| `AgentConfig` | 运行配置（mode、model、apiUrl、apiKey、systemPrompt、temperature、maxTokens） |
| `AgentContext` | 上下文快照（openFiles、fileTree、cursorPosition、selection、conversationHistory） |
| `IAgentProvider` | AI 后端插件契约（initialize、sendMessage、streamMessage、dispose） |
| `EditOperation` | 编辑操作（insert / delete / replace，基于行列定位） |
| `TextSelection` | 文本选区范围（起止行列） |
| `IAgentFileSystem` | Agent 所需的文件系统最小接口（readFile、writeFile、exists、readDir） |
