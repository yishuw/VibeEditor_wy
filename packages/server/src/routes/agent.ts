...
import { Agent, Session, executeEdits, createOpenAILLMProvider, buildMessages, McpManager, type AgentContext, type AgentConfig, type AgentEditResult } from '@vibeeditor/agent';
import { LocalFileSystem } from '@vibeeditor/core';
import { loadEnabledMcpConfig } from './mcp';
  ...
  router.post('/stream', async (req: Request, res: Response) => {
    ...
    try {
      if (config.mode === 'build') {
        ...
        // 从服务端配置文件读取启用的 MCP 服务器
        const mcpConfig = loadEnabledMcpConfig(configDir);
