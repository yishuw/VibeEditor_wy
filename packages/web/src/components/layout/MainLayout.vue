...
async function undoLastEdits() {
  ...
  for (const filePath of lastEditedFiles.value) {
    ...
    if (original !== undefined) {
      try {
        await fs.client.value.writeFile(filePath, original);
...
async function handleApplyEdits(edits: ParsedEdit[]) {
  ...
  for (const edit of edits) {
    try {
      ...
      try {
        const original = await fs.client.value.readFile(resolvedPath);
      ...
      fs.error.value = `Edit failed: ${edit.path} - ${e.message}`;
    }
  }
}
</script>
